...
def generate_og_c_code_cnn(path, name, input, cnn_details, act_details, int_weights):
    ...
    with open(path + '/' + name + '.c', 'w') as f:
        ...
        st = 1 # Maxpool,Avgpool stride index
        flatten = 0
        ...
        st = 1    # pool stride index 
        m_idx = 1 # M value index 
        dn = 1    # dense index
        flatten = 0  # flatten flag 
        act_num = 0  # activation index 
        ...
        for idx, detail in enumerate(cnn_details[:-1]):
            ...
            if detail["layer_type"] == "Conv2d":
                ...
                if(i == 1):
                    ...
                    f.write(', MV' + str(m_idx) + ', SV' + str(m_idx) + ', ' + str(relu_flag) + ');\n')
                ...
                f.write(', MV' + str(m_idx) + ', SV' + str(m_idx) + ', 0);\n')
                ...
                if detail.get("has_downsample"):
                    ...
                    f.write('\t\tshortcut_downsample(outp_dim' + str(i) + ', out' + str(main_idx))
                    f.write(', out' + str(ds_idx) + '_ds, out' + str(i))
                    f.write(', MV' + str(m_idx) + ', SV' + str(m_idx) + ', ' + str(relu_flag) + ');\n')
                    m_idx += 1
                    ...
                    f.write('\t\tshortcut_identity(outp_dim' + str(i) + ', out' + str(i-1))
                    f.write(', out' + str(i-shortcut_length-1) + ', out' + str(i))
                    f.write(', MV' + str(m_idx) + ', SV' + str(m_idx))
                    f.write(', MV' + str(m_idx+1) + ', SV' + str(m_idx+1) + ', ' + str(relu_flag) + ');\n')
                    m_idx += 2
                ...
                if flatten == 0:
                    ...
                    f.write('\t\tmlp_layer(out' + str(i-1) + ', out' + str(i) + ', flatten_dim, DENSE_DIM1')
                    f.write(', W1, B' + str(fi + dn - 1) + ', SB' + str(fi + dn - 1) + ', MV' + str(m_idx))
                    f.write(', SV' + str(m_idx) + ', ' + str(relu_flag) + ');\n')
                    dn += 1
                    flatten = 1
                else:
                    ...
                    f.write('\t\tmlp_layer(out' + str(i-1) + ', out' + str(i) + ', DENSE_DIM' + str(dn-1))
                    f.write(', DENSE_DIM' + str(dn) + ', W' + str(dn) + ', B' + str(fi + dn - 1))
                    f.write(', SB' + str(fi + dn - 1) + ', MV' + str(m_idx))
                    f.write(', SV' + str(m_idx) + ', ' + str(relu_flag) + ');\n')
                    dn += 1
